...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert 'CHANGES.rst' in found_toplevel_files
        assert '.coveragerc' in found_toplevel_files
        ...
        assert 'CONTRIBUTING.rst' in found_toplevel_files
        ...
        assert 'project_name' in found_toplevel_files
...
def test_bake_and_run_tests(cookies):
    with bake_in_temp_dir(cookies) as result:
        assert result.project.isdir()
        run_inside_dir('py.test', str(result.project)) == 0
